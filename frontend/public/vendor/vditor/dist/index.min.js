...
// [Core Patch] Vditor 骨架魔改补丁：拦截由于拉长底衬引起的退化光标防摔落陷阱
(function() {
    document.addEventListener('click', function(e) {
        var ir = document.querySelector('.vditor-ir');
        if (ir && e.target === ir) {
            var lastP = ir.lastElementChild;
            // 如果最后没有标准防弹墙 P 标签保护
            if (!lastP || lastP.tagName !== 'P') {
                lastP = document.createElement('p');
                lastP.setAttribute('data-block', '0');
                lastP.innerHTML = '<wbr>'; // 极其隐蔽的零宽占位，维持厚度
                ir.appendChild(lastP);
            }
            // 不要任何原生虚假光标，强制将焦点通过更底层的 Range 拘禁在 P 掩体内部
